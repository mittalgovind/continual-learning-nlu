...
class BertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = self.masked_softmax(self.classifier)
        self.init_weights()

    @add_start_docstrings_to_callable(BertPreTrainedModel.BERT_INPUTS_DOCSTRING)
    def forward(
            self,
            input_ids=None,
            attention_mask=None,
...
def masked_softmax(logits):
    ...
    x = select(tf.tile(tf.equal(output_mask[None, :], 1.0), [tf.shape(logits)[0], 1]), logits,
...
def estimate_fisher_mean(args, train_dataset, model, task, fisher_sample_size, fisher_consolidate, mean_consolidate,
    ...
    loglikelihood_grads = zip(
        *[autograd.grad(l, model.parameters(), retain_graph=(i < len(loglikelihood_list)), allow_unused=True) for i, l
          in enumerate(loglikelihood_list, 1)])
...
def train(args, train_dataset, task, all_tasks, model, task_num, tokenizer, accuracy_matrix, consolidate_fisher,
          consolidate_mean):
    """ Train the model """
    ...
    for i in range(len(all_tasks)):
        # Previous tasks
        if (i < task_num):
            model.load_state_dict(torch.load(os.path.join(args.output_dir, "task_paramters_" + str(i) + ".pt")),
                                  strict=False)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    features = convert_examples_to_features(
        ...
        # pad_token_segment_id=0)# if args.model_type in ["xlnet"] else 0,
    )
    ...
    dataset = TensorDataset(all_input_ids[0:data_size], all_attention_mask[0:data_size],
                            all_token_type_ids[0:data_size], all_labels[0:data_size])
    return dataset
...
def main():
    ...
    for i in range(len(configs)):
        ...
        batch_size = 2
        consolidate_fisher, consolidate_mean = estimate_fisher_mean(new_args, train_dataset, models[i][1], tasks[i],
                                                                    fisher_sample_size, consolidate_fisher,
                                                                    consolidate_mean, batch_size)
