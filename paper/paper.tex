...
% Final paper rubric  (20 points)
% This is roughly the same as the above, but adds points for results and analysis, and adds a requirement that the introduction summarizes the results.
 
% Introduction/overall idea  (7):
...
% Clear summary of findings  (3)
 
% Background  (3):
% No major omissions from the literature review  (2)
% 10+ papers cited  (1)
% 5+ relevant papers discussed in detail, with explanations of how they relate to your project  (1)
 
% Results and Analysis  (7):
% Well designed experiments with no unnecessary differences between any models/methods under comparison  (3)
% Work fully complete, with no easy-to-answer empirical questions left unanswered  (2)
% Informative, non-trivial error analysis and results writeup  (2)
 
% Overall writing  (3):
% Reasonably close fit to ACL style  (1)
% Readable prose  (2)
% Collaboration statement  (0, -1 for groups that leave it out) 
% We reserve the right to redistribute points unevenly between team members in case of very uneven contributions.
...
\begin{algorithm}
  ...
  \For{$i = 1:N$}{
    ...
    $\theta_{B_i}, \theta'_{T_i}$ \leftarrow $\mbox{argmin}_{\theta_B, \theta_T} L (D_i,  \theta_{B_{i-1}}, \theta_{T_i})$\\
    \For{$j = 1:N$}{
...
% We fine-tune BERT on $N$ downstream tasks $T_i, \forall i \in [1, N]$.  During the training loop, the task-specific training data is loaded. Let the BERT-specific parameters be $\theta_{B_i}$ after any training task $T_i$. When training the next task $T_{i+1}$, a randomly initialized task-specific head with parameters $\theta_{T_{i+1}}$ is attached to previously trained $\theta_{B_{i}}$.
...
\begin{table}[h!]
    ...
    \begin{tabular}{|c|c|c|}
        \hline
         \textbf{Task} & \textbf{MRPC$\rightarrow$RTE} & \textbf{RTE$\rightarrow$MRPC} \\
        \hline \hline
        \textbf{Base} & 0.87 (-0.48)$\rightarrow$0.64 & 0.65 (-0.14)$\rightarrow$0.83\\
        \hline
        \textbf{EWC} & 0.86 (-0.00)$\rightarrow$0.65 & 0.65 (-0.09)$\rightarrow$0.86 \\
        \hline
        \textbf{MAS} &  0.87 (-0.09)$\rightarrow$0.71 & 0.71 (-0.16)$\rightarrow$0.87 \\
        \hline
    \end{tabular}
    }
...
\begin{table}[h!]
    \centering
     \caption{High Resource Tasks: SST-2 and QNLI}
    \resizebox{0.9\columnwidth}{!}{
    \begin{tabular}{|c|c|c|}
        \hline
         \textbf{Task} & \textbf{SST-2$\rightarrow$QNLI} & \textbf{QNLI$\rightarrow$SST-2} \\
        \hline \hline
        \textbf{Base} & 0.91 (-0.39)$\rightarrow$0.77 & 0.86 (-0.38)$\rightarrow$0.90\\
        \hline
        \textbf{EWC} & 0.91 (-0.08)$\rightarrow$0.79 & 0.87 (-0.01)$\rightarrow$0.88 \\
        \hline
        \textbf{MAS} &  0.90 (-0.41)$\rightarrow$0.50 & 0.87 (-0.30)$\rightarrow$0.90 \\
        \hline
    \end{tabular}
    }
   
    \label{tab:high_resource_table}
\end{table}
...
\begin{table}[h!]
    \centering
    \caption{Three Tasks: MRPC$\rightarrow$RTE$\rightarrow$SST-2.  *Future transfer (FT) is not considered.}
    \begin{center}
    \resizebox{0.9\columnwidth}{!}{
    \begin{tabular}{| c | c | c | c | } 
    \hline
    \textbf{Task} & \textbf{MRPC} & \textbf{RTE} & \textbf{SST-2} &
    \hline\hline
    
    
     & 0.87 & *FT & *FT \\ 
    \cline{2-4}
    \textbf{Baseline} & 0.46 (-0.41) & 0.71 & *FT   \\ 
    \cline{2-4}
    ...
    \label{tab:three_tasks}
\end{table}

\begin{table}[h!]
    \centering
    \caption{Three Tasks: MRPC$\rightarrow$SST-2$\rightarrow$RTE.}
    % \caption{Three Tasks: MRPC$\rightarrow$SST-2$\rightarrow$RTE} \footnote{*Future transfer (FT) is not considered.}. \label{tab:three_tasks_2}
    \begin{center}
    \resizebox{0.9\columnwidth}{!}{
    \begin{tabular}{| c | c | c | c | } 
    \hline
    \textbf{Task} & \textbf{MRPC} & \textbf{SST-2} & \textbf{RTE} &
    \hline\hline
    
    
     & 0.87 & *FT & *FT \\ 
    \cline{2-4}
    \textbf{Baseline} & 0.41 (-0.46) & 0.91 & *FT   \\ 
    \cline{2-4}
